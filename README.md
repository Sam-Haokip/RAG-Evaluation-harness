...
There was a second methodological gap underneath the first one, addressed separately: α=0.8 had been selected by sweeping all eight values against the same 30 questions used to report its accuracy, which is tuning and evaluating on identical data. Splitting the 30 answerable questions into a stratified 15/15 tuning and holdout set, selecting α on tuning only, and then testing it on the untouched holdout gave a clean result on one axis and an unhelpful one on the other: α=0.8's accuracy held exactly (93.33% tuning, 93.33% holdout, zero gap), so the tuning process itself is not overfitting. But pure semantic search (α=1.0) scored identically on the same holdout, 93.33%. At 15 questions per half, this split cannot distinguish "hybrid genuinely helps" from "hybrid and pure semantic are tied," which is exactly what the McNemar test above already established. The holdout split validates the alpha-selection methodology; it does not validate the claim that hybrid search wins.
...
| `alpha_sweep_holdout.py` | Stratified tuning/holdout split for alpha selection, so the chosen weighting is validated on data it never influenced |
