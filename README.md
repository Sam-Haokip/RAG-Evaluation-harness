...
Seven retrieval and generation configurations, benchmarked against the same hand-verified 36-question set (factual, multi-hop, and deliberately unanswerable questions).

| Configuration | Retrieval Accuracy | Notes |
|---|---|---|
| Fixed-line chunking (baseline) | 75.00% | Original 30-question set |
| AST-based chunking, first attempt | 66.67% | Regression — see [Findings](#what-i-got-wrong) |
...
| Query decomposition | 83–96% (varies by run) | Non-deterministic — LLM-generated sub-queries |

**Cost per question:** baseline retrieval costs ~₹0.00002; query decomposition costs 100–280× more for comparable or sometimes zero accuracy gain. Hybrid search adds a full second retrieval signal (BM25) at **zero marginal cost**, since it's computed locally against the already-loaded corpus.
...
While building an LLM-as-judge to grade generated answers, the exact same evaluation pipeline scored **37.5% or 75%** depending entirely on how strictly the grading prompt was worded — no other change. Digging into *why* led to a deeper investigation: reading the disputed cases showed two distinct failure patterns hiding inside "the judge disagreed" (genuinely shallow answers vs. answers that were *more* detailed than the reference and got penalized for it), which motivated a third, source-grounded judge that checks the answer against the retrieved code directly instead of a single fixed reference phrasing.

That judge's first version was also broken — it marked 100% of answers correct, meaning it had over-corrected into never penalizing anything. Adding an explicit genericness check ("could this exact answer be reused for a different question and still sound fine?") fixed it. Full writeup in [`docs/rag-evaluation-project.docx`](docs/rag-evaluation-project.docx).
...
I'm leaving the wrong conclusion in the full writeup rather than quietly correcting it, because the correction is itself evidence the methodology can catch its own mistakes when pushed further — which is the whole point of building an evaluation harness in the first place.

## How correct is "correct"?

Retrieval accuracy only measures whether the right file was found — not whether the final generated answer was actually right. Grading that required building an LLM judge, and the project ended up being as much an investigation into *how to evaluate an LLM's answers* as into retrieval itself. See the [full writeup](docs/rag-evaluation-project.docx) for the complete four-stage judge investigation.
